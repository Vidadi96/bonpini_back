...
class Inbox_adm_model extends CI_Model
{
   ...
   public function get_inbox_data($user_id)
   {
     $query = 'SELECT i.id, i.status, i.ad_id, m.message, m.was_online, m.user_id as "user_id",
               ...
               WHERE i.status != 0 and (i.buyer_id = '.$user_id.' or i.seller_id = '.$user_id.')
               GROUP BY i.id
	 ...
	 {
		 $query = 'SELECT i.*, if(isnull(u.name), "Administrasiya", u.name) as "name", if(isnull(u.img), "", u.img) as "user_img" FROM inbox as i
		 					 LEFT JOIN (SELECT id, name, img FROM users) as u on u.id = i.user_id
							 WHERE i.id ='.$id;
