<?php
  ...
  class Inbox_adm extends MY_Controller
  {
      public function __construct()
      {
        parent::__construct();
        $this->load->model('inbox_adm_model');
      ...
      public function get_inbox_data()
      {
        ...
        if (@$this->check_token($user_id)) {
          $this->universal_model->delete_item(array('user_id' => 99999), 'open_inbox');
          $data = $this->inbox_adm_model->get_inbox_data(99999);
      ...
      public function set_online()
      {
        ...
        if (@$this->check_token($user_id)) {
          ...
          if ($online)
            $this->inbox_adm_model->set_inbox_online($user_id);
      ...
      public function get_inbox_with_data($id)
      {
        ...
        if (@$this->check_token($user_id)) {
          $messages = $this->inbox_adm_model->get_messages($page, $count, $id);
      ...
      public function bring_inbox_data($id)
      {
        ...
        if (@$this->check_token($user_id)) {
          ...
          $this->universal_model->delete_item(array('user_id' => 99999), 'open_inbox');
          $this->universal_model->add_item(array('user_id' => 99999, 'inbox_id' => $id), 'open_inbox');

          $this->inbox_adm_model->check_seen($id, 99999);

          $messages = $this->inbox_adm_model->get_messages2($page, $count, $id);
          ...
          $messages_count = $this->universal_model->get_more_item_select_row('inbox', 'count(*) as "count"', array('inbox_id' => $id));
          $seen_array = $this->inbox_adm_model->get_seen_or_not(99999);
      ...
      public function check_user()
      {
        ...
        if (@$this->check_token($user_id)) {
          $check_user = $this->universal_model->get_more_item_select_row('inbox_id', 'id, seller_id, buyer_id', '(seller_id = 99999 or buyer_id = 99999) and id ='.$inbox_id);

          $check_seen = $this->inbox_adm_model->check_if_seen(99999, $inbox_id);
          $check_was_online = $this->inbox_adm_model->check_online(99999, $inbox_id);
          ...
          $obj = $this->inbox_adm_model->get_obj($id);
